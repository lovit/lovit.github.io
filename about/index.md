...
이 글은 2019.10.3 에 마지막으로 수정되었습니다.
...
실제로 사용할 수 있는 시스템까지 만드는 것이 목표였기 때문에 계산 비용은 반드시 해결해야 할 문제였고, 이는 최인접이웃 검색을 위한 Approximated Nearest Neighbor Search (ANNS) 분야를 공부하게 된 계기가 되었습니다.
...
사람은 문서 내 여러 문장들을 복합적으로 이용하여 문서 집합을 이해하기 때문입니다.
이 과정에는 미등록단어의 이해 및 의미의 유추 과정도 포함되어 있습니다.
토크나이저 역시 이처럼 처리하는 문서에 대한 적응 능력이 있어야 한다고 생각합니다.
그때부터 하나의 문서나 한 사건에 대한 뉴스처럼 동일 주제의 문서 집합으로부터 스스로 단어를 학습하여 문장을 토크나이징 하는 방법에 대하여 연구하기 시작했습니다.
...
토픽 모델링이나 키워드 추출 같은 단어 수준의 분석을 위해서 품사 판별과 용언의 원형 복원 기능까지 제공하는 토크나이저를 연구 중입니다.
...
둘째는 텍스트 마이닝과 관련하여 한국어로 정리된 자료들을 찾는 분들이 많다는 점이었습니다.
...
개인적인 이유로 활동을 그만두게 되었는데 어린 친구들이 계속 걱정되었습니다.
...
생각을 확장해보니 그러한 순간들은 어른들이 청소년들을 사회안전망에 연결시키는 역할을 하는 순간이었습니다.
경험상 청소년기에는 경찰, 구급대, 혹은 상담가 같은 사회안전망에 연락하기가 쉽지 않습니다.
그리고 우리 사회는 다양한 사회적 약자와 함께 구성되어 있습니다.
...
"박사가 해야 할 일 중 하나는 한 분야의 공부를 시작하려는 사람이 효율적으로 공부할 수 있도록 지금까지의 연구를 정리하는 것"이라는 말에 공감하여, 더 많은 이들이 쉽게 텍스트마이닝을 공부할 수 있었으면 좋겠다는 바람에 강의자료를 정리하여 공개하였습니다.
...
"show me scary movies" 와 같은 요청에 대응하기 위하여 기존에는 컨텐츠의 장르 정보가 자주 이용되었습니다.
...
이를 돕기 위하여 한 분석팀이 보유한 동종의 데이터로 수행했던 분석들과 이에 사용된 알고리즘을 정리한 "분석 가이드라인"을 작성하는 과제를 수행하였습니다.
...
오류를 교정하기 위하여 지도 (supervised) 기반 모델을 학습할 경우 모델이 적용될 도메인이 달라질 때마다 해당 도메인에 적절한 학습데이터를 구축해야 합니다.
...
빠른 계산을 위하여 k-means 가 이용되지만, 문서의 개수가 클 경우 비효율성이 발생하며 군집화 학습 결과를 해석하기도 어렵습니다.
...
그러나 생성 (generative) 모델중 하나인 LDA 는 토픽의 대표성보다 단어의 빈도수를 기준으로 단어의 영향력이 결정되기 때문에, LDA 의 학습 결과를 해석하기 위해서는 토픽 키워드를 탐색하는 과정이 필요합니다.
...
좋은 벡터 표현법의 조건 중 하나는 "중요한 정보가 손실되지 않아 과업의 성능이 높고, 더하여 해석력을 지니는 것"이라 생각합니다.
...
많은 종류의 머신러닝 알고리즘은 소프트맥스, 유클리디언 거리, 코싸인 등 벡터의 내적을 기반으로 설계된 함수를 이용합니다.
특히 함수 결과 값의 상위 k 개만을 이용하는 최인접이웃 탐색 문제의 경우 데이터의 개수와 검색비용이 비례합니다.
하지만 추천처럼 실시간의 검색 결과를 요구하는 경우에는 낮은 검색비용이 필수입니다.
검색의 정확도가 요구되는 상황에서 iDistance 를 기반으로 하는 벡터 검색 시스템을 개발한 경험이 있으며 (NNS0, NNS1, NNS2), 고차원 벡터 공간에 적용할 수 있는 방법들을 정리하여 학회에서 튜토리얼을 발표한 경험이 있습니다 (NNS3).
