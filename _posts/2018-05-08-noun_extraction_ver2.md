...
이전 포스트에서 제안하였던 soynlp.noun.LRNounExtractor 명사 추출기를 처음 디자인했던 때는 2015년 4월 입니다. 벌써 3년 묵은 방법인데, 여러 데이터에 적용해보면서 일반화 성능에 어떤 한계가 있다는 느낌을 자주 받았습니다. 대체로 자주 등장한 명사들은 추출을 잘 하였지만, 놓쳐버린 infrequent nouns 들도 다수 있었습니다. Infrequent noun 이야 bow model 을 만들면 무시되어 사실상 신경쓰지 않았습니다만, 명사가 아닌데 명사로 추출되는 단어들이 있었습니다. Accuracy 를 높이기 위해서 후처리 과정을 더 다듬었던 것이 soynlp.noun.NewsNounExtractor 입니다. 후처리는 근본적인 해결방법이 아니라는 생각을 하였지만, LRNounExtractor 의 문제점을 유형화하지 못하여 개선 방향을 잘 잡지 못했습니다.

최근에 정말 고마운 [pull request][soynlp_pullreque] 를 받았습니다. Pull request 의 수정된 부분이 이전의 문제점을 명확히 집었을 뿐더러 명확한 말로 표현하지 못하던 LRNounExtractor 의 다른 논리적인 문제점을 정리할 수 있도록 도와줬습니다. 이를 바탕으로 오랜만에 version 2 의 명사 추출기를 만들었습니다.

이번 포스트에서는 LRNounExtractor 의 문제점에 대하여 살펴보고, 이를 해결하기 위한 논리도 함께 이야기합니다.


## Problem 1. 어미, 조사에 모두 해당하는 R parts: -은, -는

LRNounExtractor 가 이용하는 R parts 에 대한 coefficients 의 snapshot 을 살펴보면 대표적인 조사인 '-은, -는'의 coefficient 가 음수입니다. '먹 + 은', '가 + 는' 처럼 '-은, -는'은 자주 이용되는 어미이기도 합니다.
...
세종 말뭉치에서 학습한 R parts 의 coefficients 는 세종 말뭉치의 특징을 반영합니다. 만약 세종 말뭉치의 용언들이 활용될 때 '-은, -는'이 자주 이용되는 어미였다면 늘 negative coefficients 를 가질 것입니다.

그보다 더 큰 문제는 infrequent noun 의 경우 몇 개의 조사만 이용할 것입니다. 만약 A+'_' (1번), A+은(1번) 등장하였다면 A는 명사가 아닌 것으로 인식될 수 있습니다. 중요한 점은 coefficients 의 부호나 크기가 아니라 어미와 조사가 분명히 다름에도 불구하고 하나의 feature 로 취급했던 것입니다. 판단하기에 충분한 정보가 없을 때에는 억측을 하지 않는 것이 좋다고 생각합니다. 때로눈 판단하기 어려운 단어로 따로 정리하여 사용자에게 보여주는 것 만으로도 분석에 도움이 될 것입니다.

R features 각각에 대한 coefficients 를 활용하지 않습니다. 대신 R features 를 positive, negative, common features 세 종류로 분류합니다. Positive features 는 조사 혹은 '-하는'과 같은 형용사 (정확히는 형용사형전성어미 + 어말어미가 결합된 복합형태소)가 포함되어 있습니다. Negative features 에는 어근의 오른쪽에 등장하는 어미입니다. Common features 는 '-은, -는' 처럼 조사와 어미 둘 모두에 해당하는 features 입니다. 이들은 판별 과정에 영향을 주지 않아야 합니다.
...
두 가지 features 를 추가하였습니다. Unknown features 는 positive, negative, common 이 아닌 features 입니다. 우리가 아직 모르는 R features 일수도 있습니다. End features 는 L part 가 하나의 어절로 구성된 경우입니다. 명사는 조사와 결합되기도 하지만, 그 자체로 어절을 이루기도 합니다. 

이전에는 아래 예시에서의 명사의 오른쪽에 등장한 글자들인 (-는, -를, ... )을 이용하여 드라마가 명사인지 판단하였습니다. 이 점수가 어느 수준 이상이면서 그 자체로 어절을 이루는 경우가 많다면 더더욱 명사라는 확신을 가질 수 있습니다. 특히 뉴스 데이터에서는 이처럼 기관명 다음에 조사가 붙지 않는 경우가 많습니다. 대부분의 R parts 가 빈 칸이라 하더라도 다양한 조사들이 몇 번만 등장한다면 우리는 더 확신을 가지고 명사를 추출할 수 있습니다.
...
Pull request 에서 언급된 상황은 '감사함니다만'이란 어절이었습니다. '-만'가 대표적인 조사이기 때문에 '감사합니다'가 명사로 추출되었습니다. 더욱이 '감사합니-' 다음에 자주 등장하는 '-다만'은 '하 + 다만, 가 + 다만'처럼 이용되는 대표적인 어미입니다. 후처리의 규칙에서 거를 수 없는 경우였습니다.

제안해주신 방법은 -만, -다만 처럼 L parts 의 꿑부분의 글자를 살펴봤을 때 R features 로 이용되는 다른 글자가 있는 경우, 이 R 을 feature 로 이용하지 말자는 의견이었습니다. 만약 '감사합니다'가 실제로 명사였다면 '-만' 외에도 다양한 조사가 등장할 것이기 때문입니다. 

Pull request 의 전문은 [github][soynlp_pullreque] 에 있습니다.

exist_longer_pos 는 '대학생으 + 로' 와 '대학생 + 으로' 의 경우를 방지하는데 유용합니다. '대학생으'가 명사인지 판단할 때 R feature 에 -'로'가 등장하였다면, '-으로'가 positive feature 에 포함되기 때문에 counting 을 하지 않습니다. 만약 '대학생으'가 명사라면 -로 말고도 다양한 조사들이 등장할 것이기 때문에 명사 점수는 높을 것입니다. '대학생'이 명사였다면 '대학생으' 다음에는 '-로' 밖에 등장하지 않으므로 어떤 경우에도 해당하지 않게 됩니다. 이 방법은 pull request 코드에서 제안된 방법입니다.

대신 '가고있다 + 고' 처럼 '-고'에 의하여 '가고있다'라는 L part 가 명사로 추출되지 않게 만들기 위하여 더 긴 negative features 가 있을 경우에는 negative 부분에 count 를 추가합니다.
...
다섯 종류의 features 의 빈도수를 이용하여 명사 점수를 계산합니다. 이전처럼 명사 점수의 범위를 $$[-1, 1]$$ 로 만들기 위하여 기본 점수는 아래처럼 정의합니다. 

$$score(l) = \frac{\#pos - \#neg}{\#pos + \#neg}$$

이 점수가 minimum_noun_score 보다 크다면 명사로 인식합니다. 그리고 l 이 명사로 등장한 횟수는 pos + end + common 으로 정의합니다. 이전 버전의 LRNounExtractor 에서는 어절의 왼쪽에 등장한 substring count 를 명사 빈도수로 계산하였지만, 이는 정확한 빈도수가 아닙니다. '아이유 (100 번)', '아이돌 (100 번)', '아이 + 가 (100 번)' 등장하였을 경우, '아이'라는 명사의 빈도수는 100 이 되어야 하지만, 이전의 방법에서는 300 번 이었습니다. 이러한 문제를 해결하기 위하여 명사 + 조사의 형태로 어절에 등장한 경우 (positive + common) 와 명사가 단독으로 어절을 이룬 경우의 빈도수 만을 l 의 빈도수로 계산합니다.
...
위의 데이터에서 '감사합니다'라는 L part 의 오른쪽에는 주로  {'', '-만'} 처럼 특정한 R part 만 등장하는 상황이었습니다. 자주 등장한 L part 임에도 불구하고 한 두 개의 R part 가 등장한다면 명사가 아닐 것이라는 의심을 해야 합니다. 진정 명사였다면 다양한 조사가 등장할 가능성이 높습니다. min_num_of_features 라는 사용자 설정 parameter 를 따로 두어, 이 값을 넘지 않는 R features 를 지닌 단어에 대해서는 prediction 을 하지 않는 것이 더 좋다 판단하였습니다.
...
'떡볶이' vs '떡볶 + 이' 의 경우에, '떡볶이'가 명사로 판단된다면 '떡볶'은 명사로 판단할 필요가 없습니다. L parts 에 떡볶이 (100 번), 떡볶 (100 번) 등장하였다면 '떡볶-'은 '떡볶이'의 substring 일 가능성이 높습니다. 이때는 '떡볶'에 대한 명사 판별을 수행할 필요가 없습니다. 하지만 떡볶이 (100 번), 떡볶 (300 번) 등장하였다면, 떡볶 (200 번) 은 '-이' 외에도 다른 조사와 함께 등장하였는지 살펴볼 필요가 있습니다.
...
또한 명사 추출의 후보를 줄였습니다. 이전에는 모든 L parts 의 substring 에 대하여 prediction 을 수행하였습니다만, R parts 에 positive feature 가 한 개 이상 등장한 L parts 에 대해서만 명사 추출을 수행하였습니다. 이 경우에는 단일 명사가 언제나 하나의 어절을 만든 경우는 제외됩니다. 그러나 이 경우는 어자피 R features 가 존재하지 않아 prediction 을 할 수 없습니다. soynlp 의 명사 추출기는 R features distribution 을 이용하여 명사를 추출할 수 있다는 것이 기본 가정입니다. 하지만 복합명사라면 아직 기회는 있습니다 (Problem 5 참고).

아래는 2016-10-20 뉴스에서 L parts 의 substring 중 앞의 두 글자가 '아이'이며 R part 에 positive feature 가 한 개 이상 등장한 152 개의 모든 substring 에 대한 prediction score 의 변화입니다. 두 번째 column 은 명사로 추출된 어절을 L-R graph 에서 지우지 않은 경우이며, 세 번째 column 은 명사로 추출된 경우 Noun + Positive features 를 L-R graph 에서 지운 경우입니다.

'아이오아이와', '아이오아이에'는 각각 '아이오아이와 + 의', '아이오아이에 + 게'에 의하여 선택된 명사 후보입니다. 이들의 오른쪽에는 한 종류의 R feature 만 등장하여 min_num_of_features 에 의한 filtering 으로 0 점으로 처리되었습니다.

'아이오아이'는 250 번 등장하였으며, 함께 등장한 '아이오아이 + ['', 와의, 에게, 는, ...] 등은 L-R graph 에서 제거됩니다. 그 효과는 '아이오아'에서 나타납니다. 이전에는 아이오아 + [이, 이는] 때문에 명사 점수가 1 점이었습니다만, '아이오아이'가 포함된 어절이 L-R graph 에서 제거됨으로써 명사 점수가 0 점이 되었습니다.

더 극적인 경우는 '아이디' 입니다. '-어'는 대표적인 어미이기 때문에 '아이디어' 에 의하여 '아이디'의 명사 점수는 -0.526 이며, 등장 빈도수는 231 이었습니다. 하지만 '아이디어'가 명사로 추출된 이후, 이 단어가 들어간 모든 어절이 L-R graph 에서 사라짐으로써 명사 점수는 0.903 으로 올랐으며, 등장 빈도수는 100 으로 감소하였습니다. 감소한 131 번에는 '아이디어'를 포함한 다른 단어의 빈도수가 포함되어 있습니다.

그 결과 '아이'라는 단어의 명사 점수도 변화하였음을 볼 수 있습니다.
...
조사나 어미가 어절의 R part 위치에 존재하지 않으면서 어절 내에 여러 개의 명사가 존재한다면 이를 복합명사로 판단할 수 있습니다. 이 복합명사가 여러 조사들과 함께 자주 등장하였다면 그 자체가 명사로 추출될 수도 있지만, 복합명사 하나로 어절이 구성될 경우에는 R features 가 없기 때문에 앞에서 언급한 명사 추출 방법을 적용할 수 없습니다. 하지만 복합 명사를 구성하는 단일 명사 (units)를 인식할 수 있다면 예외적으로 string match 를 통하여 복합명사를 인식할 수 있습니다.

이를 위하여 soynlp.tokenizer 의 Max Score Tokenizer 를 이용하였습니다. prediction 과정에서 minimum_noun_score 이상의 점수를 얻은 명사들의 점수를 noun_scores 로 만듭니다. {'경찰', '병원', '국립'} 이 모두 명사로 추출되었다면 '국립경찰병원'은 Max Score Tokenizer 에 의하여 ['국립', '경찰', '병원']으로 나뉘어질 것입니다. 각 단어의 점수가 모두 minimum_noun_score 이상이라면 이를 복합명사로 인식하면 됩니다. 

{% highlight python %}
from soynlp.tokenizer import MaxScoreTokenizer

_compound_decomposer = MaxScoreTokenizer(scores=noun_scores)
{% endhighlight %}
...
더하여 학습된 명사 추출기를 이용하여 복합명사를 분해하는 함수를 추가하였습니다. 
...
이 작업을 하며 두 가지 utils 를 만들었습니다. EojeolCounter 는 이름 그대로 어절의 빈도수를 계산합니다. LRGraph 는 앞서 이야기한 L-R graph 입니다. EojeolCounter 에서 LRGraph 로 변환하는 함수도 만들었습니다. 이는 to_lrgraph() 를 이용하시면 됩니다. L, R 의 최대 길이를 조절 할 수 있습니다.
