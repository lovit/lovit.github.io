...
먼저 한국어 어절의 구조에 대하여 다시 한 번 살펴봅니다. 한국어의 어절은 L + [R] 구조입니다. 띄어쓰기가 제대로 되어있다면 한국어는 의미를 지니는 단어 명사, 동사, 형용사, 부사, 감탄사가 어절의 왼쪽 (L part)에 등장합니다. 문법 기능을 하는 조사나 어미가 등장합니다. 

그리고 새롭게 만들어지는 단어들은 문법 기능이 아닌 의미를 표현하는 단어입니다. 이들은 L part 에 위치합니다. 우리가 데이터 기반으로 추출해야 하는 단어는 L part 에 있습니다. R part 의 어미도 새롭게 만들어지는 단어 (정확히는 형태소) 입니다. 다양한 말투 때문에 만들어집니다. 단, 어절의 중간에 위치한 글자는 추출할 필요가 없습니다. '아이오아이'의 가운데의 '이오아'는 단어 추출의 후보에서 제외해도 됩니다. L part 도 R part 도 아니면 단어가 아닙니다. '이오아'가 단어라면 다른 어절에서 L part 일 것입니다. 
...
P(노래|노)를 정의합니다. Frequentist 처럼 관찰한 정보를 그대로 확률을 정의합니다. '노'라는 글자가 나온 다음, '노래'가 나온 경우가 100 / 200 번 이므로 P(노래|노) = 0.5 입니다. 이처럼 다른 substring 도 한 글자가 더 늘어날 확률을 각각 계산합니다. 
...
'노'는 context 로써 명확한 정보를 주지 못합니다. 어절의 종류가 많아지면 P(노x|노)의 확률은 매우 작아집니다. '노란' 이라는 substring 은 '노란색' 을 지칭합니다. '노란'은 '노란색'이라는 정보를 명확히 가르쳐주는 context 입니다. 단어의 경계 이전에는 다음 글자를 예측하기가 쉽습니다. 그러나 단어의 경계를 넘어서 존재하는 글자는 이전 글자가 명확한 context 가 되지 않습니다. '노란색'은 '노란색을'의 명확한 context 가 아닙니다. 
...
$$cohesion(c_{0:n}) = \left( \prod P(c_{0:i+1} \vert c{0:i}) \right) ^{n-1}$$

어절의 왼쪽에서부터 $$n-1$$ 개의 $$P(xy \vert x)$$ 를 누적하여 곱한 뒤 $$n-1$$ root 를 취해줍니다. $$P(xy \vert x)$$ 는 확률이기 때문에 1 이하입니다. 길이가 길수록 누적곲이 같거나 줄어듭니다. 더 긴 글자를 선호 (preference) 하기 위하여 root 를 취합니다. '노란색'의 cohesion 은 아래와 같습니다. 

$$cohesion(노란색) = P(노란 \vert 노) \times P(노란색 \vert 노란)} ^(0.5) = (0.5 \times 1) ^(0.5) = 0.707$$
...
$$cohesion(c_{0:4}) = \frac{c_{0:2}}{\cancel{c_{0:1}}} \ times \frac{\cancel{c_{0:3}}}{\cancel{c_{0:2}}} \ times \frac{c_{0:4}}{\cancel{c_{0:3}}}$$


## L-Tokenizer

Cohesion score 은 어절의 왼쪽에 위치한 substrings 중 가장 단어스러운 부분을 찾는 방법입니다. 이를 이용하면 간단한 비지도학습 기반 토크나이저를 만들 수 있습니다. 
