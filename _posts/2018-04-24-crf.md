...
이 때 각 classification 에 이용할 수 있는 features 는 potential functions 에 의하여 만들어집니다. Potential functions 에 의하여 1 의 값을 받는 features 는 매우 작습니다. 애초에 potential function 은 현재 시점이 특정한 경우인가를 판단하는 필터이기 때문입니다. 그 결과 한 시점에 대한 벡터는 대부분의 값이 0 인 sparse vector 이며, 0 이 아닌 값들도 1 을 지닙니다. 

![]({{ "/assets/figures/crf_potential_function.png" | absolute_url }})

이는 마치 5 개의 문서에 대한 term frequency vector 처럼 보이기도 합니다. Potential functions 에 의하여 feature vector 로 변형된 $$x_i$$ 는 이제 logistic regression 에 입력되어 classification 이 됩니다. 
...
![]({{ "/assets/figures/crf_mecab_label_bias.png" | absolute_url }})
...
하지만 infrequent state 였던 $$B$$ 는 언제나 $$E$$ 앞에 등장하였습니다. $$B$$ 가 정답으로 이용된 적이 적어서 $$P(B \rightarrow E)$$ 가 높을 뿐인데, 오히려 왜곡이 생깁니다. 그 결과 $$P(A, D \vert x) < P(B, E \vert x)$$ 여서 잘못된 labeling 이 이뤄집니다. 
