...
$$entropy(p(x)) = \sum -p(x_i) \cdot log p(x_i)$$
...
![]({{ "/assets/figures/tsne_perplexity_data1.png" | absolute_url }}){: width="70%" height="70%"}

![]({{ "/assets/figures/tsne_perplexity_data2.png" | absolute_url }}){: width="70%" height="70%"}

![]({{ "/assets/figures/tsne_perplexity_data3.png" | absolute_url }}){: width="70%" height="70%"}
...
Perplexity 가 작을수록 가장 가까운 bins 의 probability 는 큽니다. 그리고 $$\sigma$$ 는 작습니다. $$\sigma$$ 가 작을수록 $$\frac{\vert x_i - x_j \vert^2}{\sigma^2}$$ 의 값은 커집니다. 그리고 점들 간의 $$p_{j \vert i}$$ 의 값은 아주 가까운 점을 제외하고는 매우 작은 값이 됩니다. Exponential 은 $$exp(-0.1)$$ 과 $$exp(-0.2)$$ 는 차이가 크지만 $$exp(-10)$$ 과 $$exp(-20)$$ 은 차이가 거의 없기 때문입니다.

즉 작은 $$\sigma$$ 는 locality 를 강조합니다. 이 때는 $$p_{j \vert i}$$ 의 분포가 한 쪽으로 크게 치우칩니다. 반대로 큰 $$\sigma$$ 는 모든 점들 간의 유사도를 비슷하게 만듭니다. 그렇기 때문에 점들 간의 $$p_{j \vert i}$$ 는 큰 차이가 나지 않습니다. 
...
![]({{ "/assets/figures/tsne_perplexity_data4.png" | absolute_url }}){: width="90%" height="90%"}
